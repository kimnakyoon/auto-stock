...
document.getElementById('btnSsgStart').addEventListener('click', async () => {
  ...
  // 💡 이 버튼을 누른 '현재 탭'만 SSG 로그인 확인용으로 쓴다
  const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
  if (!tab) { alert('현재 탭을 찾지 못했습니다.'); return; }

  chrome.runtime.sendMessage({ type: 'ssg_start', tabId: tab.id, id, pw }); // 표시 갱신은 storage.onChanged 에서
});
